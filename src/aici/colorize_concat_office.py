...
def main():
    ...
    if not K_path.exists():
        raise FileNotFoundError(
            f"{K_path} not found. Please run `python3 -m aici.extract_office_samples` first."
        )
    K = np.load(K_path)
    fx, fy = K[0, 0], K[1, 1]
    cx, cy = K[0, 2], K[1, 2]
    ...
    print("Streaming bag and building world-frame colorized cloud...")
    while reader.has_next():
        ...
        if topic == lidar_topic:
            ...
            print(f"[LIDAR frame {lidar_idx}] kept {pts_world.shape[0]} world-frame colored points.")
    ...
    if not all_xyz_world:
        raise RuntimeError("No colored world points collected. Check topics, transforms, and STRIDE.")

    all_xyz_world = np.vstack(all_xyz_world).astype(np.float32)
    all_rgb = np.vstack(all_rgb).astype(np.uint8)

    print(f"Final world-frame concatenated cloud: {all_xyz_world.shape[0]} points.")

    out_ply = results_dir / "office_colorized_cloud_world.ply"
    write_ply_xyzrgb(out_ply, all_xyz_world, all_rgb)
    print(f"Saved world-frame colorized concatenated cloud to: {out_ply}")
